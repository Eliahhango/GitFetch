					...
					<div className="glass-panel p-4 md:p-6 rounded-2xl">
						<div className="flex items-center justify-between mb-6">
							<div className="flex items-center gap-3">
								...
								<div>
									<h2 className="font-headline-md text-headline-md text-on-surface">Download Queue</h2>
									{queueItems.length > 0 && (
										<p className="text-[12px] text-on-surface-variant/60">{queueItems.length} item{queueItems.length === 1 ? '' : 's'} queued</p>
									)}
								</div>
							</div>
							{queueItems.length > 0 && (
								<button
									type="button"
									className="text-[12px] font-semibold text-on-surface-variant/60 hover:text-error transition-colors flex items-center gap-1 px-3 py-2 rounded-lg hover:bg-error/5 min-h-[44px]"
									onClick={clearAll}
								>
						...
						{queueItems.length === 0 ? (
							...
							<div className="space-y-2">
								{queueItems.map((item, index) => (
									<div
										key={`${item.url}-${index}`}
										className="flex items-center gap-3 p-3.5 rounded-xl bg-black/[0.02] border border-black/[0.03] hover:bg-black/[0.04] transition-colors"
									>
